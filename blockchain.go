...
func (bc *Blockchain) CalculateTotalAmount(blockcainAddres string) float32 {
	var totalAmount float32 = 0.0
	for _, b := range bc.chain {
		for _, t := range b.transactions {
			value := t.value
			if blockcainAddres == t.recipientBlockchainAddress {
				totalAmount += value
			}
			if blockcainAddres == t.senderBlockchainAddress {
				totalAmount -= value
			}
		}
	}
